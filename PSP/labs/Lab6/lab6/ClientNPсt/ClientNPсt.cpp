...
int main(int argc, _TCHAR* argv[])
{
	...
	try
	{
		...
		DWORD bytes;
		char buffer[50] = { "Hello From ClientNPсt" };
		char* outbuffer = new char[50];
		memset(outbuffer, NULL, 50);

		//установка св€зи с именованным каналом, выполнение 1 транзакции и разрыва св€зи
		if (!CallNamedPipe(ADRESS, // канал
			buffer, //входной буфер
			sizeof(buffer), // размер входного буфера
			outbuffer, // выходной буфер
			50, // размер выходного буфера
			&bytes, // кол-во прочитанных байт
			NULL)) // параметр дл€ асинхронного доступа
		{
			throw SetPipeError("CallNamedPipe:", GetLastError());
		}
		cout << "Message from ServerNP: " << outbuffer << endl;
	}
