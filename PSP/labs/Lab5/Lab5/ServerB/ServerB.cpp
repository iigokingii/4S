...
bool  GetRequestFromClient(
	...
	if (strcmp(ibuf, name) == 0) {
		...
		char name[50];
		if (gethostname(name, 50) == SOCKET_ERROR)
			throw SetErrorMsgText("gethostname: ", WSAGetLastError());
		/////////////////////////////////////////////////////////////////

		cout << "\ngiven name: " << name; //собственное имя
	...
	try
	{
		...
		while (true) {
			if (getreq = GetRequestFromClient(name, 2000, (sockaddr*)&SOCKADR, &len)) {
				...
				SOCKADDR_IN* addr = (SOCKADDR_IN*)&SOCKADR;
				hostent* hostnameClient = gethostbyaddr((char*)&SOCKADR.sin_addr, sizeof(SOCKADR.sin_addr), AF_INET);
				cout << "\nClient hostname: " << hostnameClient->h_name;
